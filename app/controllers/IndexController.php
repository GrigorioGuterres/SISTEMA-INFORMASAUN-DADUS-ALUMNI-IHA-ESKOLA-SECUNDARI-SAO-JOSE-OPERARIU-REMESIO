...
class IndexController extends BaseController{
		...
		if(!empty($user)){
			...
			if(password_verify($password_text,$password_hash)){
				...
				if($user_status != "active"){
					return $this->login_fail("Ita Boot Nia Acount Refere Seidauk Active. Favor Contactu Ba  administrator System atu hatete info Klean/Obrigadu");
				}
				...
				return $this->login_fail("Username ho password Seidauk Loos");
			}
		}
		else{
	...
	function register($formdata = null){
		if($formdata){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$fields = $this->fields = array("username","password","email","foto","no_hp","naran_kompletu","account_status"); //registration fields
